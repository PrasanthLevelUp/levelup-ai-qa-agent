...
export const QA_KNOWLEDGE_BASE: Record<Exclude<QACategory, 'generic'>, PlannedScenario[]> = {
  authentication: [
    ...
    // ── Password-reset / account-recovery surface (KB-depth sprint) ──
    // Gated on reset vocabulary so these NEVER appear for a plain login
    // requirement — they surface only when the requirement is about resetting /
    // recovering a password via an emailed token/link.
    { id: 'auth-reset-request-valid', title: 'Reset requested with a valid email', objective: 'A reset request for a registered email is accepted and a single-use, time-limited link is emailed.', coverageType: 'positive', priority: 'P0', riskArea: 'Account recovery', conditionalOnKeywords: ['reset', 'forgot', 'recover', 'recovery'], semantics: { variableUnderTest: 'none', preconditions: 'a registered user whose email is on file', variation: 'none — a reset is requested for a valid registered email', expectedBehavior: 'the request is accepted and a single-use, time-limited reset link is emailed', requiredDataRole: 'registered_user' } },
    { id: 'auth-reset-token-valid', title: 'New password set with a valid token', objective: 'Following a valid reset link, the user sets a new password and it takes effect.', coverageType: 'positive', priority: 'P0', riskArea: 'Account recovery', conditionalOnKeywords: ['reset', 'token', 'link', 'forgot', 'recover'], semantics: { variableUnderTest: 'none', preconditions: 'a registered user holding a valid, unexpired, unused reset token', variation: 'none — a compliant new password is set via the valid token', expectedBehavior: 'the new password is accepted and takes effect for future logins', requiredDataRole: 'registered_user' } },
    { id: 'auth-reset-email-sent', title: 'Reset email / link is sent', objective: 'A reset request triggers exactly one reset email containing the recovery link.', coverageType: 'positive', priority: 'P1', riskArea: 'Account recovery', conditionalOnKeywords: ['reset', 'email', 'link', 'forgot', 'recover'], semantics: { variableUnderTest: 'none', preconditions: 'a registered user whose email is on file', variation: 'none — a reset is requested exactly once for a valid email', expectedBehavior: 'exactly one reset email containing the recovery link is sent', requiredDataRole: 'registered_user' } },
    { id: 'auth-reset-no-enumeration', title: 'Reset response does not reveal account existence', objective: 'Requesting a reset for an unknown email returns the same response as a known one — no account enumeration.', coverageType: 'negative', priority: 'P1', riskArea: 'Account enumeration', conditionalOnKeywords: ['reset', 'forgot', 'recover', 'unknown', 'enumerat'], semantics: { variableUnderTest: 'email registration status', preconditions: 'the reset request form, with both a registered and an unregistered email available', variation: 'the email is replaced with an unregistered address (the request shape stays valid)', expectedBehavior: 'the response is identical to the registered-email case and does not reveal whether the account exists', requiredDataRole: 'unregistered_user' } },
    { id: 'auth-reset-token-expired', title: 'Expired reset token is rejected', objective: 'A reset link used after its expiry window is rejected and prompts a fresh request.', coverageType: 'negative', priority: 'P0', riskArea: 'Token lifecycle', conditionalOnKeywords: ['reset', 'token', 'link', 'expire', 'forgot', 'recover'], semantics: { variableUnderTest: 'token freshness', preconditions: 'a registered user holding a valid reset token', variation: 'the token is used after its expiry window has passed (everything else stays valid)', expectedBehavior: 'the reset is rejected and the user is prompted to request a fresh link', requiredDataRole: 'registered_user' } },
    { id: 'auth-reset-token-invalid', title: 'Invalid / tampered reset token is rejected', objective: 'A malformed or tampered reset token is rejected without revealing why.', coverageType: 'negative', priority: 'P1', riskArea: 'Token lifecycle', conditionalOnKeywords: ['reset', 'token', 'link', 'forgot', 'recover'], semantics: { variableUnderTest: 'token integrity', preconditions: 'a registered user holding a reset token', variation: 'the token is replaced with a malformed or tampered value (everything else stays valid)', expectedBehavior: 'the reset is rejected without revealing why', requiredDataRole: 'registered_user' } },
    { id: 'auth-reset-token-single-use', title: 'Reset token is single-use', objective: 'A reset token already consumed cannot be reused for a second reset.', coverageType: 'negative', priority: 'P1', riskArea: 'Token lifecycle', conditionalOnKeywords: ['reset', 'token', 'link', 'single', 'reuse', 'forgot', 'recover'], semantics: { variableUnderTest: 'token reuse', preconditions: 'a registered user whose reset token has already been consumed once', variation: 'the same token is submitted a second time (everything else stays valid)', expectedBehavior: 'the second attempt is rejected because the token is single-use', requiredDataRole: 'registered_user' } },
    { id: 'auth-reset-password-mismatch', title: 'Password / confirm mismatch is rejected', objective: 'When the new password and its confirmation differ, the reset is blocked with a field-level error.', coverageType: 'negative', priority: 'P1', riskArea: 'Input validation', conditionalOnKeywords: ['reset', 'confirm', 'forgot', 'recover'], semantics: { variableUnderTest: 'password / confirmation match', preconditions: 'a registered user on the set-new-password step with a valid token', variation: 'the confirmation value is made to differ from the new password (everything else stays valid)', expectedBehavior: 'the reset is blocked with a field-level mismatch error', requiredDataRole: 'registered_user' } },
    { id: 'auth-reset-password-complexity', title: 'New password must meet complexity policy', objective: 'A new password that violates the complexity/strength policy is rejected with guidance.', coverageType: 'negative', priority: 'P1', riskArea: 'Password policy', conditionalOnKeywords: ['reset', 'complex', 'policy', 'strength', 'forgot', 'recover'], semantics: { variableUnderTest: 'new-password strength', preconditions: 'a registered user on the set-new-password step with a valid token', variation: 'the new password is made to violate the complexity policy (everything else stays valid)', expectedBehavior: 'the reset is rejected with complexity guidance and no change is applied', requiredDataRole: 'registered_user' } },
    { id: 'auth-reset-rate-limit', title: 'Reset requests are rate-limited', objective: 'Repeated reset requests for the same account are throttled to resist abuse.', coverageType: 'security', priority: 'P2', riskArea: 'Abuse resistance', conditionalOnKeywords: ['reset', 'rate', 'throttle', 'limit', 'forgot', 'recover'], semantics: { variableUnderTest: 'reset request rate', preconditions: 'a registered account and the reset endpoint', variation: 'reset requests for the same account are repeated beyond the allowed threshold (each request otherwise valid)', expectedBehavior: 'further requests are throttled / rate-limited to resist abuse', requiredDataRole: 'registered_user' } },
    { id: 'auth-reset-old-invalidated', title: 'Old password stops working after reset', objective: 'Once a reset completes, the previous password no longer authenticates and existing sessions are invalidated per policy.', coverageType: 'negative', priority: 'P1', riskArea: 'Credential lifecycle', conditionalOnKeywords: ['reset', 'old password', 'previous', 'invalidate', 'session', 'forgot', 'recover'], semantics: { variableUnderTest: 'old-credential validity after reset', preconditions: 'a registered user who has just completed a password reset', variation: 'the previous (pre-reset) password is used to authenticate (everything else stays valid)', expectedBehavior: 'the old password no longer authenticates and existing sessions are invalidated per policy', requiredDataRole: 'registered_user' } },
  ],
  crud: [
    ...
    { id: 'crud-neg-double-submit', title: 'Re-submitting the same form does not create a duplicate', objective: 'Re-submitting the same create form (a double submit / duplicate submission) does not create a duplicate record — the create is idempotent and only one record results.', coverageType: 'negative', priority: 'P1', riskArea: 'Data integrity' },
    ...
    // ── Universal data-entry obligations a senior QA always writes (KB-depth sprint) ──
    // No conditionalOnKeywords + not `always`: they emit as Standard/Deep Coverage
    // when the Negative/Edge family is selected — a balanced suite gets them, a
    ...
    // Access-control depth — a senior QA tests authorization at the ENDPOINT, not
    // just the UI. Gated on authorization language so it only appears for features
    // that actually restrict access (admin/permission/role/authorized).
    { id: 'crud-neg-direct-endpoint-authz', title: 'Direct URL / API access is authorization-checked', objective: 'A user cannot bypass the UI: a direct URL or direct API call to the create endpoint is authorization-checked server-side (endpoint access control), not just hidden in the interface.', coverageType: 'negative', priority: 'P1', riskArea: 'Authorization', conditionalOnKeywords: ['authorized', 'authorised', 'admin', 'permission', 'role', 'access', 'only'] },
    ...
    { id: 'crud-pos-upload-valid', title: 'Valid file upload is accepted and stored', objective: 'A file of an allowed type and size uploads successfully and is associated with the record.', coverageType: 'positive', priority: 'P2', riskArea: 'File handling', conditionalOnKeywords: ['photo', 'image', 'upload', 'attachment', 'file', 'document', 'avatar', 'picture'] },
    { id: 'crud-neg-upload-format', title: 'Invalid / unsupported file format is rejected', objective: 'A file with a disallowed type/extension (or spoofed content type) is rejected with a clear format error.', coverageType: 'negative', priority: 'P2', riskArea: 'File handling', conditionalOnKeywords: ['photo', 'image', 'upload', 'attachment', 'file', 'document', 'avatar', 'picture'] },
    { id: 'crud-edge-upload-oversize', title: 'Oversized / large file is rejected at the boundary', objective: 'A file above the maximum allowed size is rejected; a file at the limit is accepted (boundary behaviour).', coverageType: 'boundary', priority: 'P2', riskArea: 'File handling', conditionalOnKeywords: ['photo', 'image', 'upload', 'attachment', 'file', 'document', 'avatar', 'picture'] },
    { id: 'crud-edge-upload-corrupt', title: 'Corrupted / malformed file is handled gracefully', objective: 'A corrupted or truncated file (valid extension, invalid contents) is rejected without crashing the flow.', coverageType: 'edge_cases', priority: 'P3', riskArea: 'File handling', conditionalOnKeywords: ['photo', 'image', 'upload', 'attachment', 'file', 'document', 'avatar', 'picture'] },
  ],
  search: [
  ...
  checkout: [
    ...
    // ── Payment failure surface — every card checkout must handle these (KB-depth sprint).
    // Gated on the payment vocabulary so they only appear for card/payment flows.
    { id: 'checkout-neg-declined-card', title: 'Declined card is handled', objective: 'A card declined by the processor does not create an order; the user sees a clear decline message and can retry.', coverageType: 'negative', priority: 'P0', riskArea: 'Payment failure handling', conditionalOnKeywords: ['card', 'payment', 'pay', 'charge'] },
    { id: 'checkout-neg-expired-card', title: 'Expired card is rejected', objective: 'A card past its expiry date is rejected at validation before any charge is attempted.', coverageType: 'negative', priority: 'P1', riskArea: 'Payment validation', conditionalOnKeywords: ['card', 'payment', 'pay', 'charge'] },
    { id: 'checkout-neg-invalid-cvv', title: 'Invalid CVV / security code is rejected', objective: 'A wrong or malformed CVV / security code blocks the payment with a field-level error.', coverageType: 'negative', priority: 'P1', riskArea: 'Payment validation', conditionalOnKeywords: ['card', 'payment', 'pay', 'cvv', 'charge'] },
    { id: 'checkout-neg-insufficient-funds', title: 'Insufficient funds is handled', objective: 'A payment failing for insufficient funds does not create an order and surfaces an actionable message.', coverageType: 'negative', priority: 'P1', riskArea: 'Payment failure handling', conditionalOnKeywords: ['card', 'payment', 'pay', 'charge'] },
    { id: 'checkout-neg-invalid-card-format', title: 'Invalid card number format is rejected', objective: 'A card number failing Luhn/format validation is rejected client- and server-side before submission.', coverageType: 'negative', priority: 'P2', riskArea: 'Payment validation', conditionalOnKeywords: ['card', 'payment', 'pay', 'charge'] },
    // Order integrity / fulfilment obligations.
    { id: 'checkout-neg-quantity-exceeds-stock', title: 'Quantity exceeding available stock is rejected', objective: 'Requesting more units than are in stock is blocked with a clear message; the order is not created.', coverageType: 'negative', priority: 'P1', riskArea: 'Inventory consistency', conditionalOnKeywords: ['stock', 'inventory', 'availability', 'quantity'] },
    { id: 'checkout-pos-inventory-decrement', title: 'Inventory is decremented on successful order', objective: 'A completed order reduces available stock by the ordered quantity exactly once.', coverageType: 'positive', priority: 'P1', riskArea: 'Inventory consistency', conditionalOnKeywords: ['stock', 'inventory'] },
    { id: 'checkout-pos-confirmation-email', title: 'Order confirmation email is sent', objective: 'A confirmed order triggers a confirmation email/notification to the shopper.', coverageType: 'positive', priority: 'P2', riskArea: 'Post-order communication', conditionalOnKeywords: ['email', 'notif', 'confirmation'] },
    { id: 'checkout-edge-double-submit', title: 'Double submission does not create duplicate orders', objective: 'A double-click / duplicate submit (idempotency) creates exactly one order, not two.', coverageType: 'edge_cases', priority: 'P1', riskArea: 'Order integrity' },
    { id: 'checkout-edge-session-timeout', title: 'Session timeout during checkout is handled', objective: 'A session expiring mid-checkout is handled gracefully without a partial/orphaned order.', coverageType: 'edge_cases', priority: 'P2', riskArea: 'Flow resilience', conditionalOnKeywords: ['session', 'timeout', 'expire'] },
  ],
  payment: [

  ...
  router.post('/:id/create-pr', async (req: Request, res: Response) => {
    ...
    try {
      ...
      try {
        ...
        const filePath = testFilePath || await findTestFile(cloneDir, healing.test_name);
        if (!filePath) {
          return res.status(400).json({
        ...
        if (!fs.existsSync(absPath)) {
          return res.status(400).json({ error: `Test file not found: ${filePath}` });
        }
...
| **Status** | ${patchResult.patched ? '✅ Auto-patched' : '📋 Suggestion only'} |

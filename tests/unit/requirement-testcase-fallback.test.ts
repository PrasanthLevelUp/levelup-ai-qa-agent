 ...
 *
 * `getTestCasesForRequirement` now:
 *   1. tries the FK,
 *   2. on 0 rows, falls back to traceability_links,
 *   3. self-heals the FK so the fast path works next time.
 *
...
async function main() {
  ...
  /* ── Scenario 3: neither FK nor traceability → genuinely 0 (honest empty) ─ */
  {
    ...
    const rows = await pg.getTestCasesForRequirement(REQ, COMPANY);
    assert(rows.length === 0, 'S3: returns empty when the requirement truly has no linked cases');
  }
